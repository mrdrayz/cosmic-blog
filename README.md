...
> После перезапуска сервера прьвю всех постов убираются, потому что превью хранятся на сервере (когда сервер перезагружается, директория с изображениями не сохраняется).
